...
function SidebarResizeHandle({ className, ...props }: React.ComponentProps<"div">) {
  const { toggleSidebar, sidebarWidth, setSidebarWidth, isMobile, setIsResizing } = useSidebar()
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    startWidth.current = parseFloat(sidebarWidth) || 16
    const sidebarElement = (e.currentTarget as HTMLElement).closest("[data-slot='sidebar']")
    isLeftSide.current = sidebarElement?.getAttribute("data-side") === "left"
  ...
  const handlePointerMove = (e: PointerEvent) => {
    ...
    const deltaRem = delta / rootFontSize
    let newWidth = startWidth.current + (isLeftSide.current ? deltaRem : -deltaRem)

    const min = parseFloat(SIDEBAR_WIDTH_MIN) || 10
    const max = parseFloat(SIDEBAR_WIDTH_MAX) || 28
    newWidth = Math.min(Math.max(newWidth, min), max)

    setSidebarWidth(`${newWidth}rem`)
  }
